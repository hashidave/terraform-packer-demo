...
# This is non-trivial to deploy.  First you need an HCP boundary-worker object                              ##### 
...
resource "boundary-worker" "private-worker"{
  ...
  # The activation token on the HCP side is only available on the apply that creates the boundary-worker objevct
  # so if we later change the worker ec2 instance for any reason we have to re-create the HCP boundary-worker
  # so that we can get the token back.  I supppose we could store it in Vault but that's a task for future Dave.
  lifecycle{
     replace_triggered_by=[google_compute_instance.boundary-worker]
  }
}
...
resource "google_compute_instance" "boundary-worker" {
  ...
  resource "null_resource" "worker-provisioner" {
    triggers={
      worker-id=google_compute_instance.boundary-worker.id
    ...
    connection {
       type     = "ssh"
       user     = "ubuntu"
       host     = google_compute_instance.boundary-worker.network_interface.0.access_config.0.nat_ip
       port     = 22
